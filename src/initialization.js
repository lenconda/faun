...
export const initGlobalDependencies = function(deps) {
  if (!Array.isArray) {
    throw new TypeError('[Destruction] Param `deps` should be an array');
  }
  ...
  const globalDeps = deps.reduce((current, next) => {
    ...
    if (!name || !dep || typeof name !== 'string') {
      throw new TypeError('[Destruction] Params is in a wrong type');
    }

    if (window[name]) {
      throw new ReferenceError(`[Destruction] Dependence \`${name}\` already exist on \`window\``);
    }
